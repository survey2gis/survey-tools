...
BOOL poly_Valid(POLYAREA * p)
{
    ...
    if (GETBITS(p->contours, PLF_ORIENT) == PLF_INV || poly_ChkContour(p->contours)) {
        return FALSE;
    }

    for (c = p->contours->next; c != NULL; c = c->next)
    {
        if (GETBITS(c, PLF_ORIENT) == PLF_DIR || poly_ChkContour(c) || !poly_ContourInContour(p->contours, c) )
            return FALSE;
    }
    return TRUE;
}
