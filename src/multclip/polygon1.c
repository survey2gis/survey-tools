/*
  ...
  based on:
      ...
      (C) 2019 Benjamin Ducke: added infinite loop guards to 'Gather()'
...
int Gather(VNODE * start, PLINE ** result, J_Rule j_rule, DIRECTION initdir)
{
    ...
    *result = poly_NewContour(start->point);
    ...
    do
    {
        ...
        /* BD: abort if we hit the total vertex limit (this can also happen if we are stuck in
         * an infinite loop due to a degenerate polygon)! */
        if ( abort > VTX_LIMIT ) {
        	return err_limit;
        	fprintf(stderr,"\n*** ERROR: multclip/polygon1.c: function Gather() has hit the vertex limit of %i!\n", VTX_LIMIT);
        }
